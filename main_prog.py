...
def init_db():
    ...
    # Додавання користувача admin, якщо він не існує
    cur.execute("SELECT * FROM kunden WHERE nickname=?", ('admin',))
    if cur.fetchone() is None:
        cur.execute("INSERT INTO kunden (nickname, password) VALUES (?, ?)", ('admin', '12345'))
...
def dd():
    help_win.destroy()
def send_data(Name, Count, Category, Country, Weight):
    ...
    sql_query = "INSERT INTO Dinge (Name, Count, Category, Country, Weight) VALUES (?, ?, ?, ?, ?)"
...
def neue_ding():
    def help_win_ding():
        ...
        conf_btn = ctk.CTkButton(help_win, text="Confirm", font=("Bauhaus",18), command=lambda: send_data(Name = name_enter.get(),
...
def add_arb(nam, pasw):
    if not nam or not pasw:
        ...
        # Перевірка, чи є ім'я та пароль рядками
        if not isinstance(nam, str) or not isinstance(pasw, str):
            raise TypeError('Ім\'я та пароль повинні бути рядками')

        # Перевірка, чи не починається ім'я та пароль зі спеціальних символів або цифр
        if re.match(r'^[\d\W]', nam) or re.match(r'^[\d\W]', pasw):
            raise ValueError('Ім\'я та пароль не можуть починатися зі спеціальних символів або цифр')
...
def mainwind():
    ...
    def show_graphs():
        try:
            cur.execute("SELECT name, count FROM Dinge")
            data = cur.fetchall()
            ...
            plt.subplot(1, 2, 1)
            df.plot(kind='bar', x='Name', y='Count', ax=plt.gca(), legend=False)
            plt.title('Кількість товарів')

            cur.execute("SELECT country, count(country) FROM Dinge GROUP BY country")
            data_country = cur.fetchall()
            df_country = pd.DataFrame(data_country, columns=['Country', 'Count'])

            plt.subplot(1, 2, 2)
            df_country.plot(kind='bar', x='Country', y='Count', ax=plt.gca(), legend=False)
            plt.title('Кількість товарів по країнах')
    ...
    show_graphs_btn = ctk.CTkButton(but_frame, text="Показати графіки", command=show_graphs)
    show_graphs_btn.grid(row=0, column=3, padx=(50, 0))
